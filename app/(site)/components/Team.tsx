...
// CEO + MD lead the page; everyone else sits in the wider-team grid below.
// This split also keeps both rows visually balanced now the team is 7 (which
// wouldn't divide evenly into a single three-column grid).
const leadership = team.slice(0, 2);
const wider = team.slice(2);
        ...
        <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-16 md:mb-24">
          {leadership.map((m, i) => (
            <Reveal
              key={m.name}
              delay={i * 110}
              className="w-full md:w-[calc(33.333%_-_0.75rem)]"
            >
              <MemberCard m={m} />
            </Reveal>
          ))}
